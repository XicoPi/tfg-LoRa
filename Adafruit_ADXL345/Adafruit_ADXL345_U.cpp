...
FIFO_regs_values_t Adafruit_ADXL345_Unified::FIFO_multiByteRead(void)
{
  FIFO_regs_values_t result;
...
bool Adafruit_ADXL345_Unified::begin(uint8_t i2caddr) {
  ...
  if (_i2c)
    Wire.begin(4, 15); //enric (pins de sda i scl)
  else {
    pinMode(_cs, OUTPUT);
    digitalWrite(_cs, HIGH);
    pinMode(_clk, OUTPUT);
